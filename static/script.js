...
async function generateFlashcards() {
    ...
    if (!topic) {
        showCardError('Please enter a topic first.');
        return;
    }
...
function renderStudyPlan(plan) {
    ...
    overviewDiv.innerHTML = '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;"><i class="fas fa-lightbulb" style="font-size:20px;color:#f59e0b;"></i><span style="font-weight:700;color:var(--text);font-size:16px;">Study Plan Overview</span></div><p style="color:var(--text-secondary);line-height:1.7;font-size:14px;">' + overviewText + '</p><div style="display:flex;gap:20px;margin-top:12px;flex-wrap:wrap;"><span class="studyplan-badge primary-badge">📅 ' + (plan.days_until_exam || 0) + ' days</span><span class="studyplan-badge success-badge">⏰ ' + (plan.total_study_hours || 0) + ' hours</span></div>';
    ...
    if (plan.subjects_breakdown && plan.subjects_breakdown.length > 0) {
        ...
        subjectsHTML += plan.subjects_breakdown.map(function (s) {
            ...
            return '<div class="studyplan-subject-card" style="background:var(--surface);border-radius:12px;padding:18px;margin-bottom:12px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span style="font-weight:700;color:var(--text);font-size:15px;">' + (s.subject || 'Subject') + '</span><span style="padding:5px 12px;border-radius:20px;font-size:11px;font-weight:700;' + priorityColor + '">' + (s.priority || 'Medium') + ' Priority</span></div><p style="font-size:13px;color:var(--text-secondary);margin-bottom:6px;">⏱️ <strong>' + (s.total_hours || 0) + ' hours</strong></p>' + topicsHTML + (tipsText ? '<p class="studyplan-subject-tip" style="font-size:12px;margin-top:8px;padding:8px;border-radius:8px;">💡 <strong>Tip:</strong> ' + tipsText + '</p>' : '') + '</div>';
        }).join('');
    ...
    if (plan.weekly_plan && plan.weekly_plan.length > 0) {
        ...
        if (relevantWeeks.length > 0) {
            ...
            weeklyHTML += relevantWeeks.map(function (w, i) {
                ...
                return '<div class="studyplan-week-item" style="padding:12px 0;"><div style="font-weight:700;color:var(--primary);font-size:14px;margin-bottom:6px;">Week ' + (w.week || (i + 1)) + ': ' + (w.focus || 'Study Focus') + '</div>' + tasksHTML + '</div>';
            }).join('');
    ...
    var tipsHTML = '';
    if (plan.revision_strategy) { tipsHTML += '<div class="studyplan-revision-card" style="border-radius:12px;padding:18px;margin-bottom:12px;"><h4 style="margin-bottom:10px;font-size:14px;"><i class="fas fa-sync-alt"></i> Revision Strategy</h4><p style="font-size:13px;color:var(--text-secondary);line-height:1.7;">' + formatAnyValue(plan.revision_strategy) + '</p></div>'; }
    if (plan.exam_day_tips) { tipsHTML += '<div class="studyplan-tips-card" style="border-radius:12px;padding:18px;"><h4 style="margin-bottom:10px;font-size:14px;"><i class="fas fa-star"></i> Exam Day Tips</h4><p style="font-size:13px;color:var(--text-secondary);line-height:1.7;">' + formatAnyValue(plan.exam_day_tips) + '</p></div>'; }
